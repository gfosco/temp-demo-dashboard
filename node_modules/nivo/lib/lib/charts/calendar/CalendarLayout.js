...
var _scalePropToD3Scale = require('../../scalePropToD3Scale');

var _scalePropToD3Scale2 = _interopRequireDefault(_scalePropToD3Scale);
...
var monthPathAndBBox = function monthPathAndBBox(_ref2) {
    ...
    var w0 = _d.timeWeek.count((0, _d.timeYear)(date), date); // first week of month
    var d1 = t1.getDay(); // last day of month
    var w1 = _d.timeWeek.count((0, _d.timeYear)(t1), t1); // last week of month
 ...
 */
var memoMonthPathAndBBox = _lodash2.default.memoize(monthPathAndBBox, function (_ref3) {
    var date = _ref3.date,
...
var cellPositionHorizontal = function cellPositionHorizontal(cellSize, yearSpacing, daySpacing) {
    return function (d, yearIndex) {
        var weekOfYear = _d.timeWeek.count((0, _d.timeYear)(d), d);
...
var dayFormat = (0, _d.timeFormat)('%Y-%m-%d');
...
var CalendarLayout = function CalendarLayout() {
    return {
        ...
        compute: function compute(_ref4) {
            ...
            // time related data
            var fromDate = _lodash2.default.isDate(from) ? from : new Date(from);
            var toDate = _lodash2.default.isDate(to) ? to : new Date(to);
            var yearRange = (0, _d.range)(fromDate.getFullYear(), toDate.getFullYear() + 1);
            var maxWeeks = (0, _d.max)(yearRange, function (year) {
                return (0, _d.timeWeeks)(new Date(year, 0, 1), new Date(year + 1, 0, 1)).length;
            }) + 1;
            ...
            // ——————————————————————————————————————————————————————————————————————————————————————————————————————
            // compute cellSize
            var cellSize = computeCellSize({ width: width, height: height, direction: direction, yearRange: yearRange, yearSpacing: yearSpacing, daySpacing: daySpacing, maxWeeks: maxWeeks });
            ...
            yearRange.forEach(function (year, i) {
                ...
                days = days.concat((0, _d.timeDays)(yearStart, yearEnd).map(function (dayDate) {
                ...
                var yearMonths = (0, _d.timeMonths)(yearStart, yearEnd).map(function (monthDate) {
                    return _lodash2.default.assign({ date: monthDate }, memoMonthPathAndBBox({
                        date: monthDate,
                        direction: direction,
                        yearIndex: i,
